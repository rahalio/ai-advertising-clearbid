...
from ...utils.package_scope import pkg
from .base import WebappServicesFileGenerator


class WebappContractsGenerator(WebappServicesFileGenerator):
    """
    Generates contracts file (Zod schemas re-exports from scoped core package)
    ...
    def generate_files(
        ...
        # Barrel so domain index can `export * from "./contracts"`
        contracts_index = output_dir / "index.ts"
        contracts_index_content = f'''/**
 * {pascal_case(domain_name)} Contracts
 */
export * from "./{domain_name}.zod.schema";
'''
        write_file(contracts_index, contracts_index_content)
        files.append(contracts_index)
    ...
    def _generate_contracts_content(self, context: GenerationContext, domain_name: str) -> str:
        ...
        core_pkg = pkg("core", context.config)

        return f'''/**
 * {domain_pascal} Domain Contracts
 *
 * Re-exports Zod schemas from {core_pkg} for runtime validation.
 * This avoids duplication and ensures alignment with the API contract.
 *
 * Architecture:
 * - Single source of truth: {core_pkg}
 * - No code duplication or drift
 * - Runtime validation of API responses
 * - Used in services to validate responses
 *
 * @see {core_pkg}/{domain_name} for the source schemas
 */

import {{ {domain_name}Schemas as core{domain_pascal}Schemas }} from "{core_pkg}/{domain_name}";
...
class WebappApiTypesGenerator(WebappServicesFileGenerator):
    """
    Generates API types file (TypeScript types re-exports from scoped core package)
 ...
 *
 * API contract types come from {core_pkg}.
 * This file contains ONLY re-exports - no local API contract definitions.
 */

// ============================================================================
// API Type Re-exports (from {core_pkg})
// ============================================================================
...
export type {{
  // Add types from {core_pkg}/{domain_name}
}} from "{core_pkg}";
 ...
 *
 * Architecture Rules:
 * - API types: re-exports from {core_pkg} only
