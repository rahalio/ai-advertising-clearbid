...
class FileGenerator(BaseGenerator):
    ...
    def generate(self, context: GenerationContext) -> GenerateResult:
        ...
        # Setup output directory. pipeline.clean gates layer-level wipe;
        # generators must also opt in via should_clean(). Using OR here
        # caused webapp sibling generators to delete each other's files
        # (contracts → api_types → service → … → only index left).
        should_clean = bool(context.config.pipeline.clean) and self.should_clean()
...
class SingleFileGenerator(BaseGenerator):
    ...
    def generate(self, context: GenerationContext) -> GenerateResult:
        ...
        # Setup output directory (same AND semantics as FileGenerator)
        should_clean = bool(context.config.pipeline.clean) and self.should_clean()
        output_dir = GeneratorSetup.get_output_directory(
            context,
            generator_type=self.type,
