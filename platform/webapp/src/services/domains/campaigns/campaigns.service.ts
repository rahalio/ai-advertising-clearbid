 ...
 * Campaigns Service
 *
 * API client for campaigns domain.
...
const rawCampaignsService = {
  /**
   * List campaigns
   */
  async getCampaign(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/campaigns` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create campaign
   */
  async createCampaign(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/campaigns`;
   ...
   * Get campaign
   */
  async getCampaign(campaignId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/campaigns/${campaignId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getConversionDefinition(campaignId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/campaigns/${campaignId}/conversion-definitions`;
...
export const campaignsService = makeService(rawCampaignsService, "campaigns");
