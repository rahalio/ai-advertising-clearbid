 ...
 * Settlement Service
 *
 * API client for settlement domain.
...
const rawSettlementService = {
  ...
  async getStatement(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/settlement/statements` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getStatement(statementId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/settlement/statements/${statementId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createPayout(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/settlement/payouts`;
...
export const settlementService = makeService(rawSettlementService, "settlement");
