...
const rawInventoryService = {
  /**
   * List inventory slots
   */
  async getSlot(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/inventory/slots` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createSlot(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/inventory/slots`;
   ...
   */
  async getQuarantine(slotId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/inventory/slots/${slotId}/quarantine`;
...
export const inventoryService = makeService(rawInventoryService, "inventory");
