 ...
 * Conversions Service
 *
 * API client for conversions domain.
...
const rawConversionsService = {
  /**
   * List conversion records
   */
  async getConversion(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/conversions` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getConversion(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/conversions`;
   ...
   */
  async getVerification(conversionId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/conversions/${conversionId}/verification` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const conversionsService = makeService(rawConversionsService, "conversions");
