 ...
 * Disputes Service
 *
 * API client for disputes domain.
...
const rawDisputesService = {
  /**
   * List disputes
   */
  async getDispute(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/disputes` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getDispute(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/disputes`;
   ...
   */
  async getResolution(disputeId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/disputes/${disputeId}/resolution`;
...
export const disputesService = makeService(rawDisputesService, "disputes");
