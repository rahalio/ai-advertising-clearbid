...
const rawReportingService = {
   ...
   */
  async getCostPerAction(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/reports/cost-per-action` + (params ? `?${new URLSearchParams(params).toString()}` : '');
