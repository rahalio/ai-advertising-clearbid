...
export function createDomainIdValidator(domainPrefix, description) {
    const pattern = new RegExp(`^${domainPrefix}_[0-9a-hjkmnp-tv-z]{26}$`);
    return z
        .string()
        .length(30, 'Domain-scoped ID must be 30 characters (3 prefix + 1 underscore + 26 ULID)')
        .refine((val) => pattern.test(val.toLowerCase()), `Invalid ${domainPrefix} ID format (expected: ${domainPrefix}_ulid)`)
        .refine((val) => domainPrefix === 'usr'
        ? /^usr_[0-9a-hjkmnp-tv-z]{26}$/.test(val.toLowerCase())
        : isValidDomainId(val.toLowerCase()), 'Invalid domain ID')
        .transform((val) => val.toLowerCase())
